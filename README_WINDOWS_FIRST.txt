...
- **不會** 跳黑色 console 視窗,而是在 Windows 工具列右下角
  (system tray)出現一個紅色圓圈圖示
- 瀏覽器自動開啟 http://localhost:5050 一次
- 進入後即可看到儀表板、記錄、分析等功能

控制:
- 在工具列右下角的紅色圓圈圖示上 **左鍵雙擊** → 重開瀏覽器
- 在圖示上 **右鍵點擊** → 選單:
    [開啟血壓系統]  → 開瀏覽器到 http://localhost:5050
    [離開]          → 完全停止 server

(若工具列圖示被 Windows 隱藏在 ^ 箭頭裡,點箭頭即可看到。
 也可以從「設定 → 個人化 → 工具列 → 通知區」固定顯示。)

錯誤排查:
- 如 exe 啟動失敗,檢查 dist\bp-tracker.log 看錯誤訊息
