...
<script>
  ...
  function __applyChartDefaults() {
    ...
    // 雙擊 chart → 5 個 brush 全部 reset 到完整範圍
    // 注意:noUiSlider .set() 不會 fire 'change' event,所以不能依賴
    // propagateToOthers — 用 __setAllBrushesToRange 手動聯動所有 brush
    document.addEventListener('dblclick', (e) => {
      ...
      // 用此 chart 的 full 第一筆/最後一筆日期作為「全部」範圍,
      // __setAllBrushesToRange 內部會用 indexOf + closestDate 找其他 chart 對應 index
      if (typeof __setAllBrushesToRange === 'function') {
        __setAllBrushesToRange(labels[0], labels[labels.length - 1]);
      }
    });
  }
  ...
  // 把 5 個 brush 都設到同一個「日期範圍 [fromLabel, toLabel]」。
  // 用日期(label 字串)而非 index,因為各 chart 的 full labels 索引不同
  // (例如 weeklyChart 是週,trendChart 是日)。
  //
  // 注意:noUiSlider .set() 不會 fire 'change' event,所以原本依賴 'change'
  // 的 propagateToOthers 無法被 quick chip / dblclick 觸發。改成這裡手動
  // iterate 5 個 brush 直接 set,確保聯動。
  function __setAllBrushesToRange(fromLabel, toLabel) {
    __activeBrushSync = true;
    try {
      __BRUSH_CHARTS.forEach(canvasId => {
        ...
        const labels = (chart.__bpOriginal && chart.__bpOriginal.labels)
                       ...
                       : chart.data.labels;
        if (!labels || labels.length < 2) return;
        let bs = labels.indexOf(fromLabel);
        let be = labels.indexOf(toLabel);
        if (bs < 0) bs = __findClosestDateIndex(labels, fromLabel);
        if (be < 0) be = __findClosestDateIndex(labels, toLabel);
        if (bs < 0) bs = 0;
        if (be < 0) be = labels.length - 1;
        if (bs >= be) be = Math.min(bs + 1, labels.length - 1);
        brushEl.noUiSlider.set([bs, be]);
      });
    } finally {
      __activeBrushSync = false;
    }
  ...
  function __registerQuickChips() {
    document.querySelectorAll('.brush-quick').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (!chart) return;
        // 用 __bpOriginal.labels (full,212 entries) 計算「最後 N 天」起點;
        // 若用 chart.data.labels (sliced 後例如只剩 28 entries),__dateIndexCutoff
        // 會誤把 cutoff 對到 sliced 範圍內,跑出 2025-09 的早期資料 (bug)。
        const fullLabels = (chart.__bpOriginal && chart.__bpOriginal.labels)
                            ? chart.__bpOriginal.labels
                            : chart.data.labels;
        if (!fullLabels || fullLabels.length < 2) return;
        const total = fullLabels.length;
        const e = total - 1;
        const s = days <= 0 ? 0 : __dateIndexCutoff(fullLabels, days);
        const fromLabel = fullLabels[s];
        const toLabel = fullLabels[e];
        // 直接把 5 個 brush 都 set 到同樣的日期範圍 (不依賴 event propagation)
        __setAllBrushesToRange(fromLabel, toLabel);
      });
    });
  }
